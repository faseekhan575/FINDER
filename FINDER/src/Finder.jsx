...
import {
  FaMapMarkerAlt, FaLock, FaCheckCircle, FaSearch, FaFilter,
  FaStar, FaHeart, FaBookmark, FaChartBar, FaTimes, FaArrowLeft,
  FaSortAmountDown, FaChevronDown, FaPhone, FaEnvelope, FaGlobe,
  FaUsers, FaUserTie, FaMoneyBillWave, FaBuilding
} from "react-icons/fa";
...
function getGenderColor(g) {
  const map = { boys: "text-sky-400", girls: "text-pink-400", "co-ed": "text-purple-400" };
  return map[g] || "text-white/50";
}
...
export default function Finder() {
  const [step, setStep] = useState("form"); // form | city | area | loading | results
  ...
  /* persist liked */
  useEffect(() => {
    localStorage.setItem("likedSchools", JSON.stringify(liked));
    window.dispatchEvent(new Event("likedUpdate"));
  }, [liked]);
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) setStep("city");
  };

  const handleCitySelect = (city) => {
    if (city.locked) return;
  ...
  const filteredSchools = (() => {
    ...
    if (selectedMuridkeArea !== "all") {
      list = list.filter((s) => s.area === selectedMuridkeArea);
    }
    if (categoryFilter.length > 0) {
      list = list.filter((s) => categoryFilter.includes(s.category));
    }
    if (genderFilter.length > 0) {
      list = list.filter((s) => genderFilter.includes(s.genderType));
    }
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-white" style={{ fontFamily: "Syne,sans-serif" }}>
                    Schools in <span className="gold-text">Muridke</span>
                  </h1>
                  <p className="text-white/50 text-sm mt-1">{filteredSchools.length} schools found</p>
                </div>
                <button onClick={() => setStep("area")} className="flex items-center gap-2 text-white/40 hover:text-white/70 text-sm transition-colors">
                  <FaArrowLeft /> Change Area
                </button>
              </div>
                  ...
                  <div className="glass rounded-2xl p-5 border border-white/10 sticky top-20">
                    ...
                    <div className="relative mb-5">
                      ...
                      <input
                        ...
                        className="input-dark w-full pl-8 pr-3 py-2.5 text-xs"
                      />
                    </div>

                    {/* Area */}
                    <div className="mb-5">
                      <p className="text-white/50 text-xs font-semibold mb-2 uppercase tracking-wider">Area</p>
                      <select
                        value={selectedMuridkeArea}
                        onChange={(e) => setSelectedMuridkeArea(e.target.value)}
                        className="input-dark w-full px-3 py-2.5 text-xs appearance-none"
                      >
                    ...
                    <div className="mb-5">
                      ...
                      <div className="space-y-2">
                        {CATEGORY_FILTERS.map((f) => (
                          <button
                            key={f.value}
                            onClick={() => toggleFilter(categoryFilter, setCategoryFilter, f.value)}
                    ...
                    <div className="mb-5">
                      ...
                      <div className="space-y-2">
                        {GENDER_FILTERS.map((f) => (
                          <button
                            key={f.value}
                            onClick={() => toggleFilter(genderFilter, setGenderFilter, f.value)}
                      ...
                      <AnimatePresence>
                        {filteredSchools.map((school) => (
                          <SchoolCard
                            ...
                            liked={liked.includes(school.id)}
                            saved={saved.includes(school.id)}
                            onLike={toggleLike}
                            onSave={toggleSave}
                          />
                        ))}
            ...
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="flex gap-3">
                {compareList.length === 2 && (
                  <Link
                    to={`/compare?a=${compareList[0].id}&b=${compareList[1].id}`}
